...
# compiler flags
FLAGS='-O2 -march=native -stdpar=gpu -gpu=cc80,nofma' 

# build
BUILD='build_std_gpu_mpi_single'
...
# build the code
cmake -B $BUILD -S . -DMU_IMPL=std -DMU_ENABLE_MPI=ON -DMU_ARCH=a100 -DMU_ENABLE_SINGLE=ON -DCMAKE_CXX_COMPILER=nvc++ -DCMAKE_CXX_FLAGS="${FLAGS}" && cmake --build $BUILD --parallel
