...
TARGET real_t rain_to_graupel(real_t t, real_t rho, real_t qc, real_t qr,
  ...
  // constexpr real_t c1 = real_t{1.68}; //  coefficient for raindrop freezing
  constexpr real_t c2 =
      real_t{0.66}; // FR: 2. coefficient for immersion raindrop freezing: a_if
  constexpr real_t c3 =
      real_t{1.0}; // FR: 2. coefficient for immersion raindrop freezing: a_if
  constexpr real_t c4 =
      real_t{0.1}; // FR: 2. coefficient for immersion raindrop freezing: a_if
  constexpr real_t a2 =
      real_t{1.24E-3}; //  (PI/24)*EIR*V0R*Gamma(6.5)*AR**(-5/8)
  constexpr real_t b2 = static_cast<real_t>(13.0) / static_cast<real_t>(8.0);
  constexpr real_t qs_crit = real_t{1.e-7};

  real_t result = real_t{0.0};

  if (qr > graupel_ct::qmin && t < tfrz_rain) {
    if (t > graupel_ct::tfrz_hom) {
      if (dvsw + qc <= real_t{0.0} || qr > c4 * qc) {
  ...
  if (fmin(qi, qr) > graupel_ct::qmin &&
    ...
    result = result + a2 * (qi / mi) * pow((rho * qr), b2);
  }
